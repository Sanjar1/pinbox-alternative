...
## D-052: Stay on Railway Hobby $5/mo Indefinitely; Plan Availability Outranks the Cost Budget; Never Cancel Without Written Confirmation

- Date: 2026-08-13
- Decision: (1) The workspace **stays on Hobby $5/month permanently** until Railway states in writing that this workspace can move to the Free plan. (2) The `CLAUDE.md` rule is restructured so that **"never leave this workspace without a paid plan" outranks the $1/mo usage budget** - cost optimisation is now headroom, never a reason to cancel. (3) **Only the owner may change the plan**, and any plan change requires reading `railway.com/workspace/plans` live first; asserting what Railway's catalog offers from memory or from a doc is banned. (4) A Billing Help thread was opened with Railway asking whether Free can be enabled here (https://station.railway.com/support/trial-expired-with-no-free-plan-offered-043303db) - the answer, if it ever comes, is the only thing that may reopen this decision. Supersedes D-050's "Hobby for July only, then back to Free".
- Reason: D-050's target was executed and broke production. Hobby was cancelled, the workspace fell to a 30-day trial, the trial expired **by date** (with ~$4.31 of credit unspent) and Railway paused every deployment - ~21 h with all 41 printed posters dark and votes dropped rather than queued. The premise was false: **this account is not offered a Free plan at all** (`/workspace/plans` = Hobby/Pro only; API `hasExhaustedFreePlan: true`). Cost was never the constraint - August usage was $0.69 against a $5 plan. For a product whose URL is printed on paper, plan continuity is an availability requirement, and $5/month is far cheaper than reprinting 41 posters or another day of downtime.
- Impact: `CLAUDE.md` (section renamed "Railway plan & usage budget"; rule 1 added; old file backed up), plus the stale instruction struck from `STATUS.md`, `PROGRESS.md`, `TODO.md` and D-050 above. Railway subscription `sub_1U3rzECJoPsRzQsdkjfqfuC0` ACTIVE, billing period 2026-08-13 -> 2026-09-13, auto-renewing. Hetzner cannot substitute: the poster URL is a `railway.app` domain that only answers while a Railway service runs, and moving the DB off Railway would not reduce a flat plan fee.
...
- Date: 2026-07-05
- Decision: (1) After the free-tier grant exhaustion took ALL of production offline, the owner paid **Railway Hobby ($5/mo) for July only** as a stopgap. (2) **Serverless (App Sleeping) is enabled on `web`** and must stay enabled — it is the mechanism for fitting under the Free plan's $1.00/month grant (~$1.24/mo unoptimized, 76% = idle RAM of the always-on server; projected ~$0.4–0.6/mo sleeping). (3) ~~Target: cancel Hobby before the August cycle once measured burn < $1/mo.~~ 🔴 **SUPERSEDED 2026-08-13 — this target was acted on and broke production.** Hobby was cancelled, the workspace dropped to a 30-day trial, the trial expired *by date* (not by spending — ~$4.31 of credit was still unused) and Railway paused every deployment: ~21 h of downtime, all 41 posters dead, votes lost rather than queued. This account is not offered a Free plan at all (`/workspace/plans` = Hobby/Pro only; `hasExhaustedFreePlan: true`). **New decision: the workspace stays on Hobby $5/mo** until Railway states in writing that Free is available here (asked via support thread, see TODO.md OWNER-2). Cost was never the problem — August usage was $0.69. (4) The **production domain `web-production-370c1.up.railway.app` is frozen** like the slugs — 41 printed posters encode full URLs; the web app never migrates off Railway; only backing services (e.g. the DB) may ever move behind the same domain. (5) $1/mo is now a **hard engineering budget**: every feature is checked against RAM-minutes impact before shipping (project CLAUDE.md "Railway plan & usage budget").
...
- Date: 2026-05-24
- Decision: The admin dashboard login was simplified to a single password field ("Пароль") — email field removed. Password value: `<redacted>` stored as `TEAM_PASSWORD` env var in Railway.
